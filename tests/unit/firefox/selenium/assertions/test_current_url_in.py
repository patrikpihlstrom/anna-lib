...
class TestCurrentUrlIn(unittest.TestCase):
	...
	def test_passed(self):
		self.assertTrue(assertions.current_url_in(self.driver, 'localhost', timeout=0)['passed'])
		self.assertTrue(assertions.current_url_in(self.driver, '/test', timeout=0)['passed'])
		self.assertTrue(assertions.current_url_in(self.driver, 'test/', timeout=0)['passed'])
		self.assertTrue(assertions.current_url_in(self.driver, '/test/', timeout=0)['passed'])
		self.assertTrue(assertions.current_url_in(self.driver, 'http://localhost:8000/test', timeout=0)['passed'])
		self.assertTrue(assertions.current_url_in(self.driver, 'http://localhost:8000/test/', timeout=0)['passed'])
	...
	def test_not_passed(self):
		...
		self.assertFalse(assertions.current_url_in(self.driver, 'http://localhost:8000/page', timeout=0)['passed'])
		self.assertFalse(assertions.current_url_in(self.driver, 'http://localhost:8000/page/', timeout=0)['passed'])
