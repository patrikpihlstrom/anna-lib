...
class ElementExists(unittest.TestCase):
	...
	def test_not_passed(self):
		self.assertFalse(assertions.element_exists(driver, '#test-click-', timeout=0))
		self.assertFalse(assertions.element_exists(driver, '$css#test-click-', timeout=0))
		self.assertFalse(assertions.element_exists(driver, '$xpath//select[@name="xpath"]/option[@value="not option"]', timeout=0))
		self.assertFalse(assertions.element_exists(driver, '.get-lists', timeout=0))
		self.assertFalse(assertions.element_exists(driver, '$css.get-lists', timeout=0))
		self.assertFalse(assertions.element_exists(driver, 'a[href^="https://"]', timeout=0))
		self.assertFalse(assertions.element_exists(driver, '$cssa[href^="https://"]', timeout=0))
